...
public class EmployeesControllerTests
{
    ...
    public void Add_ShouldUseServiceAndMapResult()
    {
        ...
        _employeeServiceMock
            .Setup(service => service.HireNewAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<DateTime>(), It.IsAny<EmployeeType>()))
        ...
        _employeeServiceMock.Verify(
            service => service.HireNewAsync(createModel.LastName, createModel.FirstName, createModel.StartDate, createModel.Type),
            Times.Once);
    ...
    public void Dismiss_ShouldUseServiceAndReturnOk()
    {
        //Arrange
        EmployeeNumber employeeNumber = new EmployeeNumber(DateTime.Now, 1);
        bool withNotice = Random.Shared.NextBool();
    ...
    [Test]
    public void AppointCoach_ShouldUseServiceAndReturnOk()
    {
        //Arrange
        string employeeNumber = new EmployeeNumber(DateTime.Now, 1);

        //Act
        var result = _controller.AppointCoach(employeeNumber).Result as OkResult;

        //Assert
        Assert.That(result, Is.Not.Null);
        _employeeServiceMock.Verify(service => service.AppointEmployeeAsCoach(employeeNumber), Times.Once);
    }
}