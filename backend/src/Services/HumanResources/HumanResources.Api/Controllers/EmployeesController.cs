...
namespace HumanResources.Api.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        public EmployeesController(IEmployeeService employeeService, IEmployeeRepository employeeRepository, IMapper mapper)
        {
        ...
        {
            IEmployee? employee = await _employeeRepository.GetByNumberAsync(new EmployeeNumber(number));
        ...
        {
            IEmployee hiredEmployee = await _employeeService.HireNewAsync(model.LastName, model.FirstName, model.StartDate, model.Type);
        ...
        [HttpPost("{number}/appoint-coach")]
        [Authorize(policy: "write")]
        public async Task<IActionResult> AppointCoach(string number)
        {
            await _employeeService.AppointEmployeeAsCoach(number);
            return Ok();
        }
    }
