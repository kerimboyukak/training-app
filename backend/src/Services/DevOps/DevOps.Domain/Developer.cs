...
namespace DevOps.Domain
{
    public class Developer : Entity
    {
        ...
        public Guid? TeamId { get; internal set; }  // set the TeamId of a developer from within a Team entity (when a developer joins a team)

        private Developer(string id, string firstName, string lastName, Percentage rating)
        {
            Contracts.Require(!string.IsNullOrEmpty(id), "The identifier of a developer cannot be empty");  // EmployeeNumber: when a new developer is created, Id must match the employee in the HR domain
