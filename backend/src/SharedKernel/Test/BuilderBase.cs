...
{
    /// <summary>
    /// Base class meant for classes that can build an instance of a class with private constructors and / or private setters.  (e.g. our domain entities))
    /// </summary>
    ...
    public abstract class BuilderBase<T> where T : class
    {
        ...
        {
            _itemType = typeof(T);
            Item = (T)Activator.CreateInstance(_itemType,
        ...
        protected void SetProperty<TProperty>(Expression<Func<T, TProperty>> propertyFunc, TProperty value)
        {
            ...
            {
                LambdaExpression parentFunc = Expression.Lambda(member!.Expression!, propertyFunc.Parameters);
